...
import type { PaymentMethod, ProductUnit, UserRole } from "@pilotspos/types";
...
export function SalesClient({ role }: { role: UserRole }) {
  ...
  // Producto por peso esperando que el cajero teclee lo que marca la báscula.
  const [weighing, setWeighing] = useState<PendingWeight | null>(null);
  ...
  function addToCart(product: {
    ...
    // Por libra la cantidad la define la báscula, no el número de toques:
    // se pide el peso y se agrega la línea completa de una vez.
    if (unit === "LB") {
      if (product.stock <= 0) {
        setError(`${product.name} no tiene existencia disponible`);
        return;
      }
      setWeighing({
    ...
    setCart((prev) => {
      ...
      return [
        ...prev,
        { productId: product.id, name: product.name, unitPrice, quantity: 1, unit, stock: product.stock },
      ];
    });
  }

  /** Agrega (o reemplaza) la línea de un producto pesado con las libras capturadas. */
  function addWeighedLine(pounds: number) {
    if (!weighing) return;
    setCart((prev) => {
      const existing = prev.find((l) => l.productId === weighing.productId);
      if (existing) {
        return prev.map((l) =>
          l.productId === weighing.productId ? { ...l, quantity: pounds } : l,
        );
      }
      return [
        ...prev,
  ...
  function updateQuantity(productId: string, quantity: number) {
    setCart((prev) =>
      prev.map((l) => {
        ...
        // Los pesados admiten fracciones (0.125 lb); los de pieza, enteros desde 1.
        const floor = l.unit === "LB" ? 0.001 : 1;
        const next = Math.min(Math.max(quantity, floor), l.stock);
        return { ...l, quantity: l.unit === "LB" ? Math.round(next * 1000) / 1000 : Math.round(next) };
      }),
    );
  }
    ...
    <div className="flex h-full flex-col gap-4">
      ...
      <div className="grid flex-1 grid-cols-1 gap-4 lg:grid-cols-3">
                  ...
                  {cart.map((line) => (
                    <tr key={line.productId}>
                      ...
                      <td className="px-3 py-2 text-ink">
                        Q{(Math.round(line.unitPrice * line.quantity * 100) / 100).toFixed(2)}
                      </td>
        ...
        <div className="flex flex-col gap-4 rounded-md border border-line bg-white p-4">
          ...
          <Badge tone="neutral">{cart.length} {cart.length === 1 ? "línea" : "líneas"}</Badge>
