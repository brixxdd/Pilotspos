...
/** Quetzales: el negocio opera en Guatemala. */
...
export function CashClient({
  ...
  // El cajero abre y cierra su propio turno, pero no mueve dinero a mano ni ve
  // el desglose del negocio: sólo lo que necesita para cuadrar su caja.
  const canManageCash = canPerformAction(role, "cash.manage");
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title="Caja"
        description={`${register?.name ?? "Caja"} · abierta ${new Date(session.openedAt).toLocaleString("es-GT")}${
          openedByName ? ` por ${openedByName}` : ""
        }`}
        actions={
      ...
      <Card>
        <div className="grid grid-cols-2 gap-6 sm:grid-cols-4">
          <Stat label="Fondo inicial" value={money(summary.openingAmount)} />
          <Stat label="Ventas efectivo" value={money(summary.cashSales)} />
          {canManageCash ? (
            <>
              <Stat label="Ventas tarjeta" value={money(summary.cardSales)} />
              <Stat label="Transferencias" value={money(summary.transferSales)} />
              <Stat label="Entradas" value={money(summary.deposits)} tone="success" />
            </>
          ) : null}
          <Stat label="Retiros" value={money(summary.withdrawals)} tone="danger" />
          <Stat label="Efectivo esperado" value={money(summary.expectedCash)} />
        </div>
      </Card>
